...
contains
  ...
  !****************************************************************************
  subroutine create_volume_lobattocheb_points_cube(num_intp_per_direction, points)
    !---------------------------------------------------------------------------
    integer, intent(in) :: num_intp_per_direction
    real(kind=rk),allocatable, intent(inout) :: points(:,:)
    !---------------------------------------------------------------------------
  ...
  !> Creates Lobatto-Chebyshev points (with 3 coordinates) but 
  !! for 1D kernels.
  subroutine create_volume_lobattocheb_points_cube_1d(num_intp_per_direction, points)
    !---------------------------------------------------------------------------
    integer, intent(in) :: num_intp_per_direction
    real(kind=rk),allocatable, intent(inout) :: points(:,:)
    !---------------------------------------------------------------------------
  ...
  subroutine create_surface_cheb_points_cube(num_intp_per_direction, points, dir, align)
    !---------------------------------------------------------------------------
    integer, intent(in) :: num_intp_per_direction
    real(kind=rk),allocatable, intent(inout) :: points(:,:)
    ...
    case default
      write(logUnit(1),*) 'ERROR in create_surface_cheb_points_cube: unknown ' // &
  ...
  subroutine create_surface_lobattocheb_points_cube(num_intp_per_direction, &
    ...
    case default
      write(logUnit(1),*) 'ERROR in create_surface_lobattocheb_points_cube: unknown ' // &
        & ' face direction, stopping ... '
      call tem_abort()
    end select
  ...
  subroutine create_surface_cheb_points_cube_2d(num_intp_per_direction, &
    ...
    case default
      write(logUnit(1),*) 'ERROR in create_surface_cheb_points_cube_2d: unknown ' // &
        & ' face direction, stopping ... '
      call tem_abort()
    end select
  ...
  !****************************************************************************
  !> Create Lobatto-Chebyshev nodes on a specific face of the reference element.
  subroutine create_surface_lobattocheb_points_cube_2d(num_intp_per_direction, & 
    &                                                  points, dir, align)
    !---------------------------------------------------------------------------
    integer, intent(in) :: num_intp_per_direction
    real(kind=rk),allocatable, intent(inout) :: points(:,:)
  ...
  subroutine create_surface_cheb_points_cube_1d(points, dir, align)
    ...
    case(1) ! face in x direction, x coord is fixed
      ...
      points(pointNumber , 3 ) = 0.0_rk
    case default
      write(logUnit(1),*)'ERROR in create_surface_cheb_points_cube_1d: unknown'// &
        & ' face direction, stopping ... '
      call tem_abort()
    end select
