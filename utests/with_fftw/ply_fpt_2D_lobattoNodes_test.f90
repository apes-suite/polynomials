...
!! \author{Jens Zudrop}
program ply_fpt_2D_lobattoNodes_test
  ...
  subroutine ply_check_legToPnt_2D(power, res)
    ...
    do iPolyX = 1, maxPolyDegree+1
      do iPolyY = 1, maxPolyDegree+1
        funcIndex = posOfModgCoeffQTens(iPolyX, iPolyY,1, maxPolyDegree)
        do iVar = 1, nVars
    ...
    ! Init the FPT 
    call ply_init_legFpt_2D( maxPolyDegree = maxPolyDegree, nVars = nVars, fpt = fpt, &
      &                      lobattoPoints = .true. )
    ...
    !$OMP PARALLEL &
    !$OMP DEFAULT(shared)
    call ply_legToPnt_2D( fpt = fpt, legCoeffs = legCoeffs, pntVal = pntVal, nVars = nVars, & 
      &                   lobattoPoints = .true. )
    !$OMP END PARALLEL
    write(logUnit(10),*) 'Finished'
...
end program ply_fpt_2D_lobattoNodes_test
