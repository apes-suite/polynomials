...
program ply_fpt_memory_test
 ...
 ! res = 0.0_rk
  res = 1.0_rk
  call ply_check_memory(maxPolyDegree = 2, striplen = 3, res = res)
...
contains
  ...
  subroutine ply_check_memory(maxPolydegree, striplen, res)
    integer :: maxPolyDegree, n
    real(kind=rk), intent(inout) :: res
    integer :: striplen
    integer :: iVar, nVars
    real(kind=rk), allocatable :: legCoeffs(:,:), pntVal(:,:)
    integer :: iVal
    type(ply_legFpt_type) :: fpt    

    n = maxPolyDegree+1

    ! Create the 'LegCoeffs'-array, which actually contains integers (1, 2,...,n**3) of kind rk
    nVars = 1
    allocate(legCoeffs(n**3, nVars))
    do iVal = 1, n**3
      legCoeffs(iVal, 1) = real(iVal, kind=rk)
    end do
    ...
    allocate(pntVal( n**3, nVars))
!    write(logUnit(10),*) 'Calculating FPT ...'
    !$OMP PARALLEL &
    !$OMP DEFAULT(shared)
    call ply_legToPnt_3D( fpt = fpt, legCoeffs = legCoeffs, &
      &                   pntVal = pntVal, nVars = nVars    )
    !$OMP END PARALLEL

    write(*,*)'legCoeffs --- pntVal'
    do iVal = 1, n**3
      write(*,*),legCoeffs(iVal,1),pntVal(iVal, 1)
      if (abs(legCoeffs(iVal,1)-pntVal(iVal, 1)).gt.res ) then
        res = abs(legCoeffs(iVal,1)-pntVal(iVal, 1))
      end if
    end do
...
end program ply_fpt_memory_test
